 ...
 ********************************************************************************/
*}
{assign var="email" value=$bean->email1|trim|strtolower|md5}
...
<img 
	...
	{if empty({{sugarvar key='value' string=true}})}
	   src='http://www.gravatar.com/avatar/{$email}?d={$config}/include/images/default_user_feed_picture.png&s={{$vardef.width}}' title='Gravatar [{$bean->email1}]' alt='Gravatar'
	{else}
	...
	style='
		...
		{/if}
		{if !empty($email) && empty({{sugarvar key='value' string=true}})}	
			display:	block;
			width: auto;
			height: auto;
		{/if}
